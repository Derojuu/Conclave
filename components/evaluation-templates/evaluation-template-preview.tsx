      ...
      <div className="border-b border-black/[0.06] pb-6 dark:border-white/[0.06]">
        ...
        <div className="mt-6 grid gap-5 border-y border-black/[0.06] py-5 sm:grid-cols-3 dark:border-white/[0.06]">
          <div>
            ...
            <p className="mt-2 text-[12px] font-bold text-zinc-950 dark:text-white">
              {criteria.length}
            </p>
          </div>
          <div>
        ...
        {instructions ? (
          <div className="mt-6 border-l-2 border-indigo-500 pl-4">
            ...
            <p className="mt-2 text-[11px] leading-5 whitespace-pre-wrap text-zinc-600 dark:text-zinc-400">
              {instructions}
            </p>
          </div>
        ) : null}
          ...
          return (
            ...
            >
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  <p className="text-[10px] font-bold tracking-[0.1em] text-indigo-500 uppercase">
                    Criterion {index + 1}
                  </p>
                  <h3 className="mt-2 text-[13px] font-bold text-zinc-950 dark:text-white">
                ...
                <span className="w-fit border border-black/[0.07] px-2.5 py-1.5 text-[9px] font-bold text-zinc-500 uppercase dark:border-white/[0.07]">
                ...
                {criterion.type === "NUMERIC" ? (
                  <div className="grid gap-3 sm:grid-cols-[1fr_110px] sm:items-center">
                    ...
                    <input
                      ...
                      className="h-10 border border-black/[0.08] bg-black/[0.02] px-3 text-[12px] outline-none focus:border-indigo-500 dark:border-white/[0.08] dark:bg-white/[0.02]"
                      max={criterion.maxScore}
                      min={criterion.minScore}
                      onChange={(event) =>
                        setScores((current) => ({
                  ...
                  >
                    <button
                      className={
                        score === 1
                          ? "flex min-h-11 items-center justify-center gap-2 bg-emerald-500 text-[10px] font-bold text-white uppercase"
                          : "flex min-h-11 items-center justify-center gap-2 bg-[#EBE8E1] text-[10px] font-bold text-zinc-500 uppercase dark:bg-[#111]"
                      }
                      onClick={() =>
                        setScores((current) => ({
                          ...current,
                    ...
                    <button
                      className={
                        score === 0
                          ? "flex min-h-11 items-center justify-center gap-2 bg-rose-500 text-[10px] font-bold text-white uppercase"
                          : "flex min-h-11 items-center justify-center gap-2 bg-[#EBE8E1] text-[10px] font-bold text-zinc-500 uppercase dark:bg-[#111]"
                      }
                      onClick={() =>
                        setScores((current) => ({
